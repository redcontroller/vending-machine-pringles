class VendingMachineFunc {
  constructor() {
    ...
    this.itemList = vMachine.querySelector('.pringles-list');
  ...
  bindEvents() {
     ...
     */
    const pringlesBtns = this.itemList.querySelectorAll('button');

    pringlesBtns.forEach((pringlesBtn) => {
      pringlesBtn.addEventListener('click', (event) => {
        // 현재 잔액
        ...
        // 클릭한 콜라의 가격
        const targetElPrice = parseInt(pringlesBtn.dataset.price);
        ...
        if (balanceVal >= targetElPrice) {
          ...
          for (const item of stagedListItem) {
            ...
            if (item.dataset.item === pringlesBtn.dataset.item) {
          ...
          if (!isStaged) {
            this.stagedItemGenerator(pringlesBtn);
          }

          pringlesBtn.dataset.count--;

          // 만약 콜라의 카운트가 0 이라면
          if (parseInt(pringlesBtn.dataset.count) === 0) {
            // disabled 속성 추가 + strong 품절 태그 추가
            pringlesBtn.disabled = true;
            pringlesBtn.insertAdjacentHTML(
              'afterbegin',
